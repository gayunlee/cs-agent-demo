...
## 분류 enum (반드시 이 중 하나만)
- **refund_request**: 환불 요청 ("환불해주세요", "돈 돌려주세요"). **단, 자동결제 맥락이면 auto_payment_complaint 우선**.
- **cancel_method_inquiry**: 해지 방법 문의 ("해지하려면 어떻게?", "다음 결제 해지")
- **cancel_check**: 이미 해지 신청을 **완료**한 후 처리 상태를 묻는 문의 ("해지 처리 됐나요?", "해지신청했는데 처리 확인").
  제외: **자동결제 맥락은 cancel_check 아님 → auto_payment_complaint**. 해지 방법 문의는 cancel_method_inquiry.
- **auto_payment_complaint**: 자동/정기결제 관련 불만 또는 취소/환불 요청.
  **핵심: "자동" / "정기결제" / "연장" 단어가 트리거되면 뒤에 "해지", "환불", "취소" 같은 단어가 붙어 있어도 이 유형**.
  예: "자동으로 결제됐어요", "연장되어 결제", "왜 결제됐죠?", "자동결제 됐는데 해지/환불 부탁"
...
- **refund_delay**: 이미 접수된 환불의 지연/상태 문의 ("환불 언제 되나요?", "환불 안 왔어요")
...
2. **트리거 우선순위** (여러 단어 섞여 있을 때):
   - "자동결제"/"정기결제"/"연장" 언급 → **auto_payment_complaint** (최우선. "해지/환불" 같이 있어도 이 유형)
   - "카드" 관련 변경 → **card_change** (product_change보다 우선)
   - "환불" 단독 (자동결제 맥락 없음) → **refund_request** 또는 세부 유형
3. **이전 대화 맥락**도 고려. 매니저가 이미 환불 견적 안내한 후 유저가 "네 진행해주세요" → refund_request (T2 후속)
4. **다중 의도**: 가장 중심 되는 것 하나 선택. 진짜 여러 도메인이면 compound_issue.
5. 애매하면 other, 뻔하지 않은 경우 other를 두려워하지 말 것.

## 주요 구분 예시
- "어제 자동결제가 된 이번달부터 구독 해지 및 환불 부탁드립니다"
  → **auto_payment_complaint** ("자동결제" 트리거. "해지/환불"은 결과 요청일 뿐)
- "지난주 해지 신청했는데 처리 됐나요?"
  → **cancel_check** (자동결제 맥락 없음, 이미 완료된 해지 확인)
- "환불 신청했는데 취소할게요"
  → **refund_withdrawal** (환불 요청 철회)
- "카드로 결제방법 변경"
  → **card_change** (product_change 아님)
- "환불해주세요"
  → **refund_request** (자동결제 언급 없음)
