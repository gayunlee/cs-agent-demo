...
## 22:29 — Smoke test vs strands-evals/AgentCore Evaluation 역할 구분

wrapper agent 멀티턴 디버깅 중 Gayoon이 "smoke test 왜 smoke test이냐" + "strands-evals랑 AgentCore Evaluation 쓰면 smoke 필요 없지 않냐" 질문. 역할 구분 명확화.

**읽은 파일**: `scripts/smoke_wrapper_agent.py`, `scripts/smoke_wrapper_multiturn.py`, `src/agents/wrapper_agent.py`

**의사결정**: smoke 유지 + strands-evals 에 wrapper 경로 추가 + AgentCore Evaluation 은 쇼케이스용 1개만. 세 레이어 모두 유지.
- 대안 1 (smoke 폐기): strands-evals 만으로 대체 → 거부. 이유: strands-evals 는 legacy `RefundAgentV2.process()` 직접 호출, wrapper 거치지 않음. wrapper 회귀를 잡으려면 eval driver 확장 필요하고, 설령 확장해도 evals 는 매 커밋 돌리기엔 느림/비쌈.
- 대안 2 (evals 폐기): 해커톤 쇼케이스 가치 날아감. 심사관한테 "품질 지표" 숫자 제시 불가.

**수치**:
- smoke 실행 시간: 2건 멀티턴 약 10초, 8건 single 약 60초 (사용자 로컬)
- strands-evals 실행 시간: 수치 미확인 (legacy 직접 호출, LLM judge 별도 호출)
- 현재 strands-evals 결과: **TypeAccuracy 7/8, AmountAccuracy 6/8** (직전 커밋 기준, wrapper 아닌 legacy 직접 경로)

**레이어별 역할 표**:

| 레벨 | 묻는 것 | 속도 | 출력 | 주기 |
|---|---|---|---|---|
| Smoke | "연기 나냐? tool 호출됐냐?" | 초 단위 | pass/fail | 매 수정 |
| Integration | "legacy+wrapper 연결 정확?" | 초~분 | pass/fail | 매 커밋 |
| Evaluation (strands-evals / AgentCore) | "품질 지표 몇 점?" | 분~시간 | 숫자 | nightly, 릴리스 전 |

**코드 발견**:
- `scripts/smoke_wrapper_agent.py:run_case` — `agent.handle_turn(message)` 로 호출하게 수정, Memory 경로 거치도록. return 타입이 str 이어서 `isinstance(result, str)` 분기 추가.
- `scripts/smoke_wrapper_multiturn.py:main` — `handle_turn` 으로 통일, `agent.memory` 연결 상태 print ("연결됨" / "없음 fallback") 추가.

**배운 것**:
- "Smoke" 는 회로 기판 유래 은어 — 전원 켜고 연기 안 나면 OK → 그 다음 깊은 테스트. SW 에서도 같은 의미 (최소 생존 확인).
- Smoke 와 evaluation 은 같은 축 다른 정밀도가 아니라 **완전히 다른 목적**:
  - Smoke = 개발 피드백 루프 (빠른 깨짐 감지)
  - Evaluation = 품질 트렌드 지표 (느린 숫자 생산)
- "evals 하나로 대체" 는 피드백 루프 속도 저하 + 단위 테스트 결여 → 개발 사이클 망가짐. 두 레이어 유지 원칙은 프로덕션에서도 보편.
- strands-evals 가 legacy 직접 경로를 타고 있는 구조적 이슈 발견 → wrapper 층 회귀는 smoke 또는 wrapper-path eval 확장 필요. wrapper 가 "껍데기 + 검증된 legacy 뇌" 구조라서 skip 해도 된다고 착각하기 쉽지만, SYSTEM_PROMPT / memory 주입 / turn_log 등 wrapper 자체 로직이 회귀 위험원임.
